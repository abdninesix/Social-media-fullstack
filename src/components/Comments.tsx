...
const Comments = ({ comments, postId, username }: { comments: commentWithDetails[], postId: number, username: string }) => {
  ...
  useEffect(() => {
    if (state.success) {
      socket.emit("sendNotification", { receiverUsername: user?.username, data: { senderUsername: user?.username, type: "comment", link: `/${user?.username}/status/${postId}` } })
    }
    ...
    <div className=''>
      {user &&
        (<form action={formAction} className='flex items-center justify-between gap-4 p-4 '>
          ...
          <button disabled={isPending} className="py-2 px-4 font-bold bg-white text-black rounded-full cursor-pointer disabled:opacity-75">{isPending ? "Replying" : "Reply"}</button>
        </form>)}
