...
const Recommendations = async () => {
  ...
  const friendRecommendations = await prisma.user.findMany({
    where: { id: { not: userId, notIn: followedUserIds }, followings: { some: { followerId: { in: followedUserIds } } } },
    take: 3,
            ...
            <div className='flex items-center justify-between' key={person.id}>
              ...
              <div className='flex items-center gap-2'>
                <div className='relative rounded-full overflow-hidden w-10 h-10'>
                  <Image path={person.img || "sm/general/avatarNew.png"} alt={person.username} w={100} h={100} tr={true} />
                </div>
                <div className=''>
