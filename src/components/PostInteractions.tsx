...
const PostInteractions = ({ postId, postUsername, count, isLiked, isRePosted, isSaved }: { postId: number, postUsername: string, count: { likes: number; comments: number; rePosts: number }, isLiked: boolean, isRePosted: boolean, isSaved: boolean }) => {
  ...
  const [state, setState] = useState({
    ...
    rePosts: count.rePosts, isRePosted: isRePosted,
    isSaved: isSaved,
  })

  const [optimisticCount, addOptimisticCount] = useOptimistic(state, (prev, type: "like" | "rePost" | "save") => {
    if (type === "like") {
      return { ...prev, likes: prev.isLiked ? prev.likes - 1 : prev.likes + 1, isLiked: !prev.isLiked }
    };
    if (type === "rePost") {
      return { ...prev, rePosts: prev.isRePosted ? prev.rePosts - 1 : prev.rePosts + 1, isRePosted: !prev.isRePosted }
    };
    if (type === "save") {
  ...
  const saveAciton = async () => {
    addOptimisticCount("save")
    await savePost(postId)
    setState(prev => { return { ...prev, isSaved: !prev.isSaved } })
  }
